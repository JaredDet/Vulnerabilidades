...
def _process_repository(repository: Repository, token: str, root: Path,
    ...
    try:
        source = clone_repository(
            repository, root / "repositories", token=token)
    except CloneError as error:
        result.status, result.error = "clone_failed", str(error)
        return result
    try:
        result.detected_languages = get_repository_languages(
            repository.full_name, token)
    ...
    result.languages = [
        a.language for a in result.analyses if a.status == "analyzed"]
    result.findings = [f for a in result.analyses for f in a.findings]
    failures = [a for a in result.analyses if a.status != "analyzed"]
    result.status = "analyzed" if not failures else (
        "partial" if result.languages else failures[0].status)
    result.error = "; ".join(
        f"{a.language}: {a.error}" for a in failures) or None
    return result
...
def scan_organization(organization: str, token: str, output: Path, *,
    ...
    if not organization.strip() or not token.strip() or timeout <= 0:
        raise ValueError(
            "Se requiere organización, GITHUB_TOKEN y timeout positivo")
    ...
    report = OrganizationResult(
        organization=organization.strip(), repositories=[])
    for index, repository in enumerate(sorted(repositories, key=lambda r: r.full_name), 1):
        ...
        try:
            result = _process_repository(
                repository, token, root, available, executable, timeout)
        except Exception as error:
